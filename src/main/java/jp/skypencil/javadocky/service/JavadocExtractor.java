...
public class JavadocExtractor {
  ...
  Flux<Tuple2<String, Flux<ByteBuffer>>> unzip(File file) {
    ...
    Flux<Tuple2<String, Flux<ByteBuffer>>> result =
        Flux.<Tuple2<String, Flux<ByteBuffer>>>create(
            emitter -> {
              AtomicInteger opened = new AtomicInteger(0);
              // When we use emitter (FluxSink), we need to consider how we should handle
              // cancellation
              emitter.onCancel(
                  () -> {
                      ...
                      .doFinally(
                          signal -> {
                            if (opened.decrementAndGet() == 0) {
                              // Closing ZipFile will close all InputStream created from it, so we
                              // need to wait until all InputStreams are closed
                              try {
                                zip.close();
                              } catch (IOException e) {
                                log.warn("Failed to close ZIP input stream", e);
  ...
  private Tuple2<String, Flux<ByteBuffer>> handleEntry(ZipEntry entry, InputStream input)
      throws IOException {
